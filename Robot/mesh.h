...
namespace mini
{
	...
	public:
		...
		static Mesh SimpleTriMesh(const DxDevice& device, const std::vector<VertexType> verts, const std::vector<unsigned short> idxs, const D3D_PRIMITIVE_TOPOLOGY top = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST)
		{
			if (idxs.empty())
				return {};
			Mesh result;
