...
mini::dx_ptr<ID3D11HullShader> mini::DxDevice::CreateHullShader(std::vector<BYTE> hsCode) const
{
	ID3D11HullShader* gs = nullptr;
	auto hr = m_device->CreateHullShader(hsCode.data(), hsCode.size(), nullptr, &gs);
	dx_ptr<ID3D11HullShader> geometryShader(gs);
	if (FAILED(hr))
		THROW_DX(hr);
	return geometryShader;
}

mini::dx_ptr<ID3D11DomainShader> mini::DxDevice::CreateDomainShader(std::vector<BYTE> dsCode) const
{
	ID3D11DomainShader* gs = nullptr;
	auto hr = m_device->CreateDomainShader(dsCode.data(), dsCode.size(), nullptr, &gs);
	dx_ptr<ID3D11DomainShader> geometryShader(gs);
	if (FAILED(hr))
		THROW_DX(hr);
	return geometryShader;
}
